...
pool.connect()
  ...
  .catch(err => {
    ...
    console.log('⚠️  将在无数据库模式下运行（仅测试API）');
  });
    ...
    if (adminCheck.rows.length === 0) {
      ...
      await pool.query(
        `INSERT INTO users (username, password, user_type, email, is_active) 
         VALUES ($1, $2, $3, $4, $5)`,
        ['admin', hashedPassword, 'admin', 'admin@footballbetting.com', true]
      );
...
app.get('/api/invitation-codes', async (req, res) => {
  try {
    ...
    if (codes.length === 0) {
      codes = [
        { code: 'TEST123', created_by: 'system', created_at: new Date().toISOString(), is_active: true },
        { code: 'TEST456', created_by: 'system', created_at: new Date().toISOString(), is_active: true }
      ];
    }
...
// 6. 导入邀请码（需要认证）
app.post('/api/invitation-codes', authenticateToken, async (req, res) => {
  try {
...
app.use((err, req, res, next) => {
  ...
  // CORS错误特殊处理
  if (err.message.includes('CORS') || err.message.includes('跨域')) {
    return res.status(403).json({
      success: false,
      error: '跨域请求被拒绝',
      details: err.message,
